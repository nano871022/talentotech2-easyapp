...
echo "Building Lambda package for advise-service using Docker..."
...
docker build -f "$SCRIPT_DIR/Dockerfile.lambda-build" -t advise-service-builder "$SCRIPT_DIR"

# Create a temporary container and copy files
echo "Creating build container..."
CONTAINER_ID=$(docker create advise-service-builder)
